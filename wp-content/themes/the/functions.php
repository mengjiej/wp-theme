...
add_action( 'init', 'redirect_wp_admin' );
    ...
    if ( is_admin() && is_user_logged_in() && !current_user_can( 'manage_options' ) && !current_user_can( 'publish_pages' ) && !current_user_can( 'publish_posts' ) &&!current_user_can( 'level_1' ) && ( !defined( 'DOING_AJAX' ) || !DOING_AJAX )  ){
    wp_safe_redirect( '/user-2' );
    	exit;
    	}
    }
