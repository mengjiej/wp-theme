...
function _download_file($file_dir)
{
    ...
    #div_background{
        ...
        background-image: url("https://api.btstu.cn/sjbz/api.php?method=zsy&lx=dongman");
...
#div_nr{
    ...
    border-radius: 16px;
    border: 4px solid #9a9a9a08;
    max-width: 700;
    font-size: 18;
}

#p_bt{
    background-color: #a2e1e3a1;
    margin-top: 0;
...
#bt_3{
    ...
    if (substr($file_dir,0,2) == '*[') {
        ...
        preg_match($gzea,$dwp,$gzea);//截取
        
        $gzea = explode("*|",$gzea[1]);//分割
        
        $inof = explode("|#|",$dwp);//分割
        
        echo $html_donw;//输出头部
        
        $her = '<p id="p_bt">'.$gzea[0].'</p><p id="p_sm">'.$gzea[1].'</p><p id="bt_1">解压密码</p><p id="jymm">'.$gzea[2].'</p>';
        
        
        echo $her;
        
        $vip_donw = '<p id="bt_2">VIP下载</p>';
        $free_donw = '<p id="bt_3">普通下载</p>';
        
        //循环输出
        foreach ($inof as $inofsj){
            $wplj = "/【(.*?)】/i";
            preg_match($wplj,$inofsj,$wp);//截取
            $xwplj = explode("|",$wp[1]);//分割
            if ($xwplj[0] == 'vip') {
                if ($vip_stat == '1') {
                    $vip_donw = $vip_donw.'<p><a href="'.$xwplj[2].'">'.$xwplj[1].' - 提取码：'.$xwplj[3].'</a></p>';
                } else {
                    $alert_ts = "'您不是VIP,无法使用此下载渠道'";
                    $vip_donw = $vip_donw.'<p><a href="#" onclick="alert('.$alert_ts.')">'.$xwplj[1].' - 提取码：'.$xwplj[3].'</a></p>';
                }
                
                
            } else {
                $free_donw = $free_donw.'<p><a href="'.$xwplj[1].'">'.$xwplj[0].' - 提取码：'.$xwplj[2].'</a></p>';
            }
        }
        if ($vip_donw != '<p id="bt_2">VIP下载</p>') {
            echo $vip_donw;
        }
        
        if ($free_donw != '<p id="bt_3">普通下载</p>') {
            echo $ptxz.$free_donw;
        }
        
        echo "</div></div></div></body></html>";//输出尾部
